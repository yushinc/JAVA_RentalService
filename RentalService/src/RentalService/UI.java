...
public class UI {
	public static void main(String[] args) throws Exception {
		...
		while (true) {
			try {	
				System.out.print("프로그램을 사용해본적이 있습니까? (맞으면 Y/y를 아니면 N/n을 입력하세요.) : ");
				String answer = scan.nextLine();
				// 과거 실행 여부와 파일 존재 여부 예외처리
				if (answer.equals("Y") || answer.equals("y")) { // 프로그램 첫번째 실행이 아니라면
					try { // 파일이 존재하는 경우
						ois = new ObjectInputStream(new FileInputStream("SaveData.txt")); 
						System.out.println("이전 실행 데이터를 불러옵니다.");
						act = new Manager(ois); // file read
						ois.close();
						break;
							
					} catch (FileNotFoundException fnfe) { // 파일이 존재하지 않는 경우
						System.out.println("파일이 현 디렉토리에 존재하지 않아 불러올 수 없습니다. 프로그램을 초기화 후 실행합니다.");
						act = new Manager();
						break;
						// 출력 후 초기상태의 프로그램 실행
					}
				...
				else if (answer.equals("N") || answer.equals("n")) { // 프로그램 첫번째 실행인 경우
					try { // 파일이 존재하는 경우
						ois = new ObjectInputStream(new FileInputStream("SaveData.txt")); 
						act = new Manager(ois); // 파일의 기존데이터가 존재하므로 적용하여 실행 
						System.out.println("이전 실행 데이터가 존재하므로 적용합니다.");
						ois.close();
						break;
						
					} catch (FileNotFoundException fnfe) { // 파일이 존재하지 않는 경우 (첫번째 실행이므로 존재하지 않음)
						System.out.println("프로그램 첫번째 실행입니다."); 
						act = new Manager();
						break;
						// 출력 후 초기상태의 프로그램 실행
					}
					...
					throw new IOException ("잘못된 방법의 체크아웃입니다.");	
			}
			
			catch (Exception e) {
				System.out.println();
				System.out.println("Y/y, N/n 중 하나를 입력해주세요");
				System.out.println();
			}
		}
		...
		{			
			...
			case 3: // 전체 상품 보여주기
				...
				if (act.productAt(0) == null)
					...
					for(int i = 0; i < act.getProductCount(); i++)
					{
						...
						String code = act.productAt(i).getCode();							int number = act.productAt(i).getNumber();
			...
			case 4: // 재고 상품 보여주기
				...
				// act.productAt(0) == null 은 indexOutOfBounds exception 발생
				// arrayList는 크기를 정해주지않아도 필요에 따라 추가 할당 받는다. 
				// 원소가 하나도 없으면 공간을 할당 받지 않아 예외 발생하는 것 같아 코드 변경
			...
			case 5: // 체크인 하기
				try {
					...
					// 전화번호 입력받기
					System.out.printf("체크인 하실 분의 전화번호를 적어주세요 (하이픈 포함 입력): ");
					...
					// 반납 예정 일자 입력받기
					System.out.printf("반납 예정 일자를 적어주세요(yyyy-MM-dd 형식으로 입력) : ");
					String returnDay = scan.nextLine();
						
					// User 객체 생성
					User u1 = new User(name, phone, rentalDay, returnDay);
					...
					if(rentalNum >= 4)
						throw new Exception("상품은 3개까지만 대여가능합니다.");
					...
					for(int i = 0; i < rentalNum; i++)
					{
						...
						int amount = act.productAt(index).getPrice(); // 입력받은 코드에 해당하는 인덱스를 통해 상품의 가격 구하기
						u1.addProductToUser(rentalCode, amount); // 대여 물품 코드/가격 배열에 코드/가격 추가
						// 대여하려는 물품의 가격 amount = (User의 addProduct의 money 인수)
					}
			...
			case 6: // 체크인 정보 보여주기
				...
				if (act.userAt(0) == null) // 원소가 없을 경우
					...
					for(int i = 0; i < act.getUserCount(); i++)
					{
						...
						// 유저 배열에 존재하는 유저들이 빌린 상품들의 코드 출력
			...
			case 7: // 체크아웃 하기
				...
				try {
					scan.nextLine();
					System.out.println();
					System.out.printf("대여하신 분의 전화번호를 입력하세요 (하이픈 포함 기재) : ");
					String phone = scan.nextLine();
